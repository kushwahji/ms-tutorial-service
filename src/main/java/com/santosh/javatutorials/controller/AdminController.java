...
public class AdminController {
	...
	public String update(@PathVariable(name = "id") String id, @RequestParam(name = "status") boolean status, Model model) {
		adminService.updateStatus(Integer.parseInt(id),status);
		return "redirect:/view-menu";
	}
	
	@GetMapping("/tupdate/{id}")
	public String updateTopic(@PathVariable(name = "id") String id, @RequestParam(name = "status") boolean status, Model model) {
	...
	public String editTopic(@PathVariable("id") long id, TopicDto menu, Model model) {
		...
		model.addAttribute("menus", pageService.allMenu().stream().filter(f->f.getMenuId().equals(pageService.getTopic(id).getMenuId())).collect(Collectors.toList()));
		return "admin/add-topic";
	}
